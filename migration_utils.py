    ...
    if p_keys:
        ...
        try:
            with connection.cursor() as cursor:
                # SQL Execution Timeout (cx_Oracle 구버전 호환)
                if cx_Oracle: 
                    try:
                        # 최신 버전 (8.2+)
...
def migrate(table_name, p_keys, date_column_name, fetchsize, start_date, end_date, source_conn, target_conn, update_callback=None, hint_index_column=None, index_scan_gap_minutes=0):
    ...
    try:
        ...
        with source_conn.cursor() as s_cur:
            ...
            # 쿼리 실행 타임아웃 설정 (cx_Oracle 구버전 호환)
            if cx_Oracle:
                try:
                    if hasattr(s_cur, 'connection') and hasattr(s_cur.connection, 'callTimeout'):
                        s_cur.connection.callTimeout = 60000
                except:
                    logging.warning(f"[{table_name}] Query Timeout 설정 실패 (무시됨)")
